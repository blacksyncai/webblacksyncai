...
export default async function handler(req: Req, res: Res) {
  // Wraps the entire handler so nothing in here — including a bug in a
  // response branch itself — can escape as an unhandled crash instead of
  // the JSON error this function intends to always send. See
  // api/signup/start.ts for the incident that motivated this.
  try {
    if (req.method !== "POST") {
    ...
    console.error("signup/complete: unhandled error escaped inner handling", outerErr);
